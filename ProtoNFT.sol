...
contract ProtoNTF is ERC721, ERC165, ERC721Metadata, ERC721Enumerable {
    ...
    function mint() public payable {
        require(msg.value >= 0.001 ether, "You need to pay 0.001 ETH");
        _lastId++;
        _ownerOf[_lastId] = msg.sender;
        _balanceOf[msg.sender]++;
        _uris[_lastId] = "http://localhost/ntf";
    ...
    function burn(uint256 _tokenId) public {
        ...
        require(owner != address(0), "Address does not exits");
        require(
            _isAprovedOrOwner(owner, msg.sender, _tokenId),
    ...
    function _isAprovedOrOwner(
        ...
        return
            ...
            _isApprovedForAllFunction(_owner, _spender);
    }

    function balanceOf(address _owner) external view returns (uint256) {
        require(_owner != address(0), "Must have an address");
        return _balanceOf[_owner];
    }

    function ownerOf(uint256 _tokenId) external view returns (address) {
        return _ownerOfFunction(_tokenId);
    }

    function _ownerOfFunction(
        uint256 _tokenId
    ...
    function _transferFrom(
        ...
        require(
            _isAprovedOrOwner(_ownerOf[_tokenId], msg.sender, _tokenId),
            "You do not have permission"
        );
    ...
    function approve(address _approved, uint256 _tokenId) external payable {
        address owner = _ownerOfFunction(_tokenId);
        require(
            owner != msg.sender || _isApprovedForAllFunction(owner, msg.sender),
            "Not authorized"
        );
    ...
    function isApprovedForAll(
        ...
        return _isApprovedForAllFunction(_owner, _operator);
    }

    function _isApprovedForAllFunction(
        address _owner,
        address _operator
    ) internal view returns (bool) {
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256) {
        require(_index < _allTokens.length, "Index out of bound");
